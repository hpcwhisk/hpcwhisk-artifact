...
DESIRED_JOB_COUNT=100

BASE_DIR=$(dirname $(readlink -f $0))
JOB_TEMPLATE="job_template.sh"

while true; do
	JOB_COUNT=$(squeue -u $USER -t pd -h | wc -l)
	RUNNING_JOB_COUNT=$(squeue -u $USER -t r -h | wc -l)
	echo "Current number of pending jobs: ${JOB_COUNT}/${DESIRED_JOB_COUNT}, running: ${RUNNING_JOB_COUNT}/${JOB_COUNT}, press [CTRL+C] to stop monitoring..."
